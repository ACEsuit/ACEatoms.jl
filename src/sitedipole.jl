...
"""
`struct ACESiteDipole` : wraps an ACE model into a JuLIP calculator
"""
struct ACESiteDipole{ENV, TM} <: SitePotential
   models::Dict{AtomicNumber, TM}
end

struct ACESiteDipoleBasis{ENV, TM} <: JuLIP.MLIPs.IPBasis
   models::Dict{AtomicNumber, TM}  # model = basis
   inds::Dict{AtomicNumber, UnitRange{Int}}
end

ACESiteCalc{ENV} = Union{ACESiteDipolel{ENV}, ACESiteDipolelBasis{ENV}}

Base.length(ipbasis::ACESiteDipoleBasis) = sum(length, values(ipbasis.models))

cutoff(V::ACESiteCalc) =  maximum(cutoff, values(V.models))

function ACESiteDipole(models::Dict{Symbol, TM}, 
                          ENV = AtomicEnvironment) where {TM} 
   mods = Dict([ AtomicNumber(sym) => mo for (sym, mo) in models]...)
   return ACESiteDipole(mods, ENV)
end

ACESiteDipole(models::Dict{AtomicNumber, TM}, ENV = AtomicEnvironment) where {TM} = 
     ACESiteDipole{ENV, TM}(models)

function _get_basisinds(V::ACESiteDipole)
   inds = Dict{AtomicNumber, UnitRange{Int}}()
   i0 = 0
...
function basis(V::ACESiteDipole{ENV}) where ENV 
