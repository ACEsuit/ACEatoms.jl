...
"""
Electrostatic forces of a set of point charges and point dipoles
calculated using the soft core potentials. 
λ = 1.0 returns the non-soft core version.
"""
...
function soft_coulomb(Rij::AbstractVector, q1::Real, q2::Real, λ::Real=0.9, α::Real=10.0)
  r = norm(Rij)
  return q1 * q2 / (4*π*ϵ_0 * (α * (1 - λ)^2 + r^2)^0.5) * e * 1e10
end

function force_q_q(Rij::AbstractVector, q1::Real, q2::Real, λ::Real=0.9, α::Real=10.0)
